...
app = Flask(__name__)
app.secret_key = os.getenv("FLASK_SECRET", "devkey")  # required for flash messages
...
FONT_PATH = "/Library/Fonts/Arial Unicode.ttf"  # macOS font path
...
def serial_exists_in_supabase(serial: str) -> bool:
    try:
        result = supabase.table("tickets").select("serial").eq("serial", serial).limit(1).execute()
...
def upsert_ticket_to_supabase(
    ...
    return supabase.table("tickets").upsert(payload, on_conflict="serial").execute()
...
def generate_ticket_image_file(serial: str, category: str) -> Path:
    ...
    draw.rectangle([(qr_x, qr_y), (qr_x + 250, qr_y + 250)], outline="gold", width=5)
    ticket_image.paste(qr, (qr_x, qr_y))
    draw.text((text_x, text_y), serial, font=font, fill="gold")
    draw.text((text_x, category_y), CATEGORIES[category]["name"], font=font, fill="gold")
...
def send_email_with_attachment(serial: str, buyer_email: str, image_path: Path) -> None:
    msg = EmailMessage()
...
def start_ticket():
    ...
    return render_template('start.html', serial=serial)


@app.route("/ticket/<serial>")
def ticket_page(serial):
    get_or_create_ticket(serial)
    # render a dedicated template for nicer UI
    return render_template('ticket.html', serial=serial, categories=CATEGORIES)
...
def pay_page(serial):
    ...
    if request.method == 'GET':
        ...
        # Extract data from existing ticket
        ...
        if category not in CATEGORIES:
            # redirect back to ticket page with a message
    ...
    razorpay_order = razorpay_client.order.create({
        "amount": total_price * 100,  # Razorpay expects amount in paisa
        "currency": "INR",
...
def confirm_payment(serial):
    ...
    # Check for Razorpay payment verification
    ...
    if request.method == "GET" and not payment_verified:
        # Show payment confirmation page using template (for manual UPI flow)
        cat = CATEGORIES[ticket["category"]]
        total_price = ticket.get("total_price", cat['price'] * ticket.get('quantity', 1))
        return render_template(
            'confirm.html',
            serial=serial,
    ...
    if request.method == "POST" or payment_verified:
        ...
        image_public_url = f"{PUBLIC_BASE_URL}/static/tickets/{image_path.name}"
        ...
        email_status = "pending"
        try:
            send_email_with_attachment(serial, ticket["buyer_email"], image_path)
            email_status = "sent"
        except Exception as e:
            print("Email error:", e)
        ...
        # render final confirmation template
        cat = CATEGORIES[category]
        total_price = ticket.get("total_price", cat['price'] * ticket.get('quantity', 1))

        return render_template(
            'ticketconfimation.html',
            serial=serial,
            category_name=cat['name'],
            category_key=category,
            price=total_price,
...
def admin_refund(serial):
    ...
    if not check_admin_key(key):
        return "Forbidden", 403
    try:
        supabase.table("tickets").update({"refunded": True}).eq("serial", serial).execute()
    except Exception as e:
...
def privacy_policy():
    ...
    <head>
        ...
        <style>
            body { font-family: 'Poppins', sans-serif; background: linear-gradient(135deg, #0f0c29, #302b63, #24243e); color: #fff; margin: 0; padding: 2rem; max-width: 800px; margin: 0 auto; }
            h1 { color: #ffd700; text-align: center; margin-bottom: 2rem; }
            h2 { color: #ffea80; margin-top: 2rem; }
            p, li { line-height: 1.6; }
            a { color: #ffd700; text-decoration: none; }
            a:hover { text-decoration: underline; }
            .back-link { display: inline-block; margin-top: 2rem; padding: 1rem 2rem; background: rgba(255,215,0,0.1); border: 1px solid #ffd700; border-radius: 8px; color: #ffd700; text-decoration: none; }
    ...
    <body>
        ...
        <p>We collect information you provide directly to us, such as when you purchase tickets: your email address.</p>
        ...
        <h2>Contact Us</h2>
        <p>If you have questions about this Privacy Policy, please contact us at <a href="mailto:privacy@aavadingi.com">privacy@aavadingi.com</a></p>
...
def terms_and_conditions():
    ...
    <body>
        ...
        <p>By purchasing a ticket to AAVADINGI 2026, you agree to these terms and conditions.</p>
        
        <h2>Refund Policy</h2>
        <p>All ticket sales are final. No refunds will be provided except in cases of event cancellation by the organizers.</p>
        
        <h2>Event Changes</h2>
        <p>We reserve the right to make changes to the event schedule, venue, or program. Ticket holders will be notified of any significant changes.</p>
        ...
        <h2>Liability</h2>
        <p>AAVADINGI organizers are not liable for any loss, damage, or injury occurring at the event.</p>
        
        <h2>Contact Information</h2>
        <p>For questions about these terms, please contact us at <a href="mailto:info@aavadingi.com">info@aavadingi.com</a></p>
